...
|------|--------|-------------------------|
| [Enhancing Text2Cypher with Schema Filtering](https://arxiv.org/html/2505.05118) (Ozsoy, 2025) | Prompt-time schema pruning (exact-match, NER-masked, similarity) to cut noise, hallucinations, and token cost when the model writes Cypher | Complementary for the **app / agent layer**: exact-match subset + full-schema fallback lives in [`examples/llm/subset.py`](https://github.com/edgesentry/ontographia/blob/main/examples/llm/subset.py) / [`pipeline.py`](https://github.com/edgesentry/ontographia/blob/main/examples/llm/pipeline.py) ([issue #45](https://github.com/edgesentry/ontographia/issues/45)). Ontographia’s core still never receives raw Cypher from the model. |
...
Measured stress-test baseline (silent wrong fields still compile on large ontologies; subset prompts shrink token cost; H1 mechanical **support**): [Evaluation — Track A](evaluation.md#track-a-distractor-ontology-stress-test) · [H1 study](evaluation.md#h1-study-full-vs-subset-vs-subsetfallback).
